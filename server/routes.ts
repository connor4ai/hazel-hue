...
async function processColorAnalysisWorker(jobId: number) {
  try {
    ...
    // Generate PDF report
    const pdfPath = await htmlPdfService.generatePremiumReport(order, analysisResult);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/orders/:orderId/pdf", async (req, res) => {
    try {
      ...
      const pdfPath = await htmlPdfService.generatePremiumReport(order, order.analysisResult);
